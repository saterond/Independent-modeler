...
This document describes the structure of Indepmod Class Notation plugin for Netbeans. The plugin is created in Netbeans platform so if you are not familiar with it, please take a look at it at first.
...
API Module provides an public API (Interfaces) that can be found (e.g. by other
Netbeans plugins) in the Lookup of the workspace (part of the Editor Module).

\subsection{Tool Chooser Module}

This module is used for setting of demanded Tool of actual selected workspace. The module has a panel with single tools - e.g. Class or Relation between classes. This module will be called only ToolChooser for simplicity.

\subsection{Editor Module}

This is the main module which is used as a workspace. Workspace has a JGraph inside which does all the work.

\section{API Module}

This module is consisted of Interfaces which implementations can be found in the lookup of the workspace (workspace is the part of the editor module). There are two packages in this module:
...
This module is very simple and is used for setting of demanded tool of active workspace. The structure can be seen in the Figure \ref{f-ToolChooserModuleToolChooser}.
...
ToolChooser is a singleton which is inherited from the TopComponent. TopComponent searches for instance of the ToolChooserModel in the lookup of active workspace and when it is found, ToolChooser can be used to set the desired tool of that workspace.
